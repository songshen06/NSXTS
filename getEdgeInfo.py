...
def GetEdgeList(nsxm_host,nsxm_user,nsxm_pass,nsxm_enpass):
	...
	time.sleep(1)
        ...
        for eachList in new_edge_list:
            ...
            command_configuration_list = ['show edge '+x +' configuration ospf\n' for x in eachList]
            command_ospf_nei_list = ['show edge '+x + ' ip ospf neighbor\n' for x in eachList]
            for command in command_route_list:
                ...
                #time.sleep(1)
                #print stdout.read()
            for command in command_configuration_list:
                ssh_conn.send(command)
                time.sleep(3)
            for command in command_ospf_nei_list:
                ssh_conn.send(command)
                time.sleep(3)
            output = ssh_conn.recv(90000)
	    print output
